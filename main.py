...
def get_user(tg_id: str, name="", username=""):
    if tg_id not in users:
        users[tg_id] = {"balance": 1000, "name": name, "username": username}
    return users[tg_id]
...
def spin_logic(tg_id: str):
    ...
    return {"roll": roll, "index": roll_index, "win": win, "balance": user["balance"]}
...
def format_users(users: dict) -> str:
    ...
    for i, (tg_id, user) in enumerate(users.items(), start=1):
        text += f"<b>{i}.</b> ID: <code>{tg_id}</code> 💰 {user['balance']}\n"
    return text
...
@app.post("/login")
def login(data: dict):
    return get_user(str(data["tg_id"]), data.get("tg_name", ""), data.get("tg_username", ""))
...
async def start(message: Message):
    get_user(str(message.from_user.id), message.from_user.full_name, message.from_user.username or "")
...
async def main():
    # создаем обе задачи в одном loop
